...
function raporYazdir(k: Yakit, plaka: string) {
  ...
  const html = `
    ...
    <body>
      ...
      <div class="sub">3D InCerTa — Araç Takip Sistemi</div>
  ...
  async function loadData() {
    ...
    const [{ data: y }, { data: a }] = await Promise.all([
      supabase
        ...
        .select("yakit_id, arac_id, tarih, yakit_turu, litre, birim_fiyat, toplam_tutar, km_bilgisi, istasyon_adi, yapilan_is, araclar(plaka)")
          ...
          <tbody>
            {kayitlar.map((k) => (
              <tr key={k.yakit_id} className="border-b border-line last:border-0">
                ...
                <td className="px-5 py-3 text-slate-500 max-w-[180px] truncate" title={k.yapilan_is ?? ""}>
                  {k.yapilan_is || "—"}
                </td>
